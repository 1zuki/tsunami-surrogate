...
Each model--target pair is defined by a single configuration file. The primary configurations train the Fourier Neural Operator on the hydrostatic, MUSCL-HR, and Boussinesq labels, and the CNN and U-Net baselines are trained on the shallow-water targets under the matched protocol above. Hydrostatic is the main reference target, while MUSCL-HR and Boussinesq are the main comparison targets; Boussinesq is included because it gives a weakly dispersive baseline on the same scenarios, not because it is treated as final physical truth.
