...
Uncertainty quantification relies on a seven-member deep ensemble of the baseline FNO, differing strictly in initialization seed. To evaluate seed sensitivity across architectures and target labels, a replicated subset uses seeds $\{18, 36, 67\}$ for Hydrostatic FNO, F-FNO, ConvLSTM, and U-Net, as well as FNO trained on MUSCL-HR and Boussinesq labels. Sample-scaling runs use seed 18 so that their full-data comparator has the same training seed.

\subsection{Hardware and runtime setting}
\label{subsec:train-hardware}

Dataset generation and the final non-timing evaluation use university cluster resources, with CUDA model evaluation assigned to a DGX-A100 node. Runtime benchmarks are conducted separately on a local system equipped with an Intel Core i5-13420H CPU and an NVIDIA GeForce RTX 4050 Laptop GPU. Training-time comparisons are omitted because they depend on the available hardware and system load.

Reported inference runtimes reflect hardware-specific benchmarks against our vectorized NumPy/SciPy reference solvers, rather than fully optimized GPU-accelerated operational codes. Random seed allocations are structured as follows: primary direct and windowed models use seed 18; the replicated subset uses seeds $\{18, 36, 67\}$; dataset scaling experiments use seed 18; and the 7-member uncertainty ensemble employs seeds $\{11, 22, 33, 44, 55, 66, 77\}$. Complete configuration files and checkpoint logs are provided for reproducibility.
