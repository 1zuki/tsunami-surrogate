...
The windowed configurations use the same hydrostatic data but change the supervised task from a direct $50$-frame forecast to a $K = 5$ frame block forecast conditioned on recent surface-elevation frames. The reported Window-FNO-5 and Window-FFNO-5 rollouts are seeded with the first target frame at evaluation time and scored on the remaining $49$ predicted target frames, so they are used as drift diagnostics and are not treated as identical to the direct single-pass prediction problem.
