...
The default forward benchmark uses a uniform $64\times64$ Cartesian grid on normalized coordinates, with matched bathymetry, source, and solver resolution. The main configuration generates $8192$ scenarios with seed $42$. Each trajectory is advanced for $250$ solver steps, every fifth state is saved, and the initial state is included, giving $51$ stored frames per rollout. Automatic time stepping is enabled with a target CFL value of $0.45$ for the shallow-water references. The default solver set contains a hydrostatic-reconstruction shallow-water solver and a second-order MUSCL-HR shallow-water solver. A Boussinesq branch is generated through a separate diagnostic configuration with smaller source amplitudes and a stricter target CFL value, and is treated as exploratory unless a particular experiment explicitly uses it.

% TODO: Replace planned scenario counts with final generated counts before submission.
...
The separate Boussinesq diagnostic configuration uses smoother bathymetries, lower source amplitudes, and a smaller source-strength range. This is done to keep the weakly dispersive branch numerically stable enough for diagnostics, rather than to define the main training distribution.
...
Preprocessing converts the raw rollout archives into model-ready arrays. The default split is $70\%/15\%/15\%$ for training, validation, and testing, using a fixed seed. The same scenario partition is reused across solver targets, so hydrostatic and MUSCL-HR datasets can be compared without changing the underlying bathymetry-source split.
...
For cross-resolution transfer, additional native $32\times32$, $64\times64$, and $128\times128$ configurations are provided. These configurations keep the same procedural generation logic while changing grid spacing, sponge width, and, for the finest grid, the target CFL value. The shared-from-$64$ preprocessing mode can force lower- or higher-resolution datasets to use normalization statistics from the $64\times64$ training distribution. This gives a stricter transfer setting: the model is evaluated on changed grid resolution without recomputing feature scaling from the target resolution.
