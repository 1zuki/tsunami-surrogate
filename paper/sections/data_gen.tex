...
The raw sample stores the source template $q$, the scalar \texttt{source\_strength} $s$, the scaled perturbation \texttt{eta0}, the \texttt{initial\_depth} $h_0$, and \texttt{free\_surface0} separately. The default model input channels are bathymetry, the raw source template $q$, and initial water depth $h_0$; the scaled perturbation is not used as the default source channel. The initial free surface can also be exported as an ablation channel, and a scalar solver-identity channel is available for explicit multi-reference conditioning experiments. The default target variable is the free-surface elevation $\eta$. With the default rollout settings, preprocessing discards the stored initial frame from the prediction target and keeps the next $50$ saved frames as a multi-step forecast target.
