...
The numerical evidence combines broader numerical-method and independent-comparator studies with current checks on the dataset used for the surrogate experiments. Table~\ref{tab:app-validation-chain} expands the summary in Table~\ref{tab:solver-validation}; the current checks re-audited all train, validation, and test splits of the three datasets and reran nine representative solver cases, while the broader numerical studies remain separately scoped. The 50 learning targets are adjacent-natural-step common-time outputs; no solver-step-indexed output is mixed into the reported evaluation.
...
\begin{tabular}{p{0.25\linewidth}p{0.49\linewidth}p{0.17\linewidth}}
    ...
    Internal numerical checks & $86$ cases; $48$ analytical, numerical, and stability criteria fixed before evaluation & met criteria \\
    Independent comparisons & $3$ primary comparisons, $2$ verification checks, and $7$ refinement checks & met criteria \\
    Descriptive comparison & $12$ exploratory GeoClaw/Clawpack comparisons & $11$ outside limits \\
    Integrated procedure checks & $90$ cases plus $3$ deterministic repeatability checks & repeatable \\
    Held-out sensitivity checks & $363$ paired cases and $726$ solver executions under $42$ criteria & met criteria \\
...
The exploratory finite-horizon shallow-water comparisons are the main qualification on the independent-comparator evidence. Across three prespecified representative cases, trajectory relative $L_2$ against GeoClaw is $0.414$--$0.537$ for Hydrostatic and $0.231$--$0.259$ for MUSCL-HR. The controlled ablation in Table~\ref{tab:geoclaw-ablation} shows that refinement is the dominant tested sensitivity, while a material residual remains. The numerical program therefore supports the specific numerical procedure and refinement checks needed for the benchmark, but does not establish pointwise equivalence, general boundary independence, formal convergence, or event-scale validity.
