...
All accuracy numbers are reported on the held-out test split ($N = 2500$ scenarios). Unless noted, MAE and RMSE are in denormalized benchmark-scale surface-elevation units obtained by reversing the target normalization, relative $L_2$ is dimensionless, and max-err is in normalized target units. Following Section~\ref{subsec:eval-metrics}, the relative $L_2$ and max-err values are accumulated globally over the whole split rather than averaged per batch. The FNO is trained and evaluated separately against each of the three numerical references; the expanded hydrostatic comparison set includes FFNO, mode-count ablations, U-FNO, WNO, U-Net, and CNN models.

\subsection{Reference-solver health checks and dataset quality}
\label{subsec:res-dataset}

All three numerical references use the same scenario design: parallel hydrostatic, MUSCL-HR, and Boussinesq datasets of $13{,}500$ scenarios each, split into $10{,}000$ train, $1{,}000$ validation, and $2{,}500$ test rollouts with fixed, disjoint seeds. Every target is a $50$-frame $\eta$ trajectory on a $64\times64$ grid with the $C=3$ input stack of Section~\ref{subsec:forward-task}. Under the quality policy of Section~\ref{subsec:reference-validation}, all $13{,}500$ scenarios in each dataset pass with \texttt{quality\_status}~$=$~\texttt{ok}: no rollout is dropped for non-finite values, sub-tolerance depth, or amplitude violations, and the Boussinesq conjugate-gradient acceleration solve converged on every sample. Source strength spans the same $[0.45,\, 0.85]$ range, with the six source families and five bathymetry types matched across references and splits, so each surrogate sees the same scenario population. Because scenario IDs are shared, the cross-solver gaps of Section~\ref{subsec:res-solver-fidelity} use all $2{,}500$ shared test scenarios for each pair, but they are same-scenario, same-saved-index released-label diagnostics rather than interpolated common-physical-time comparisons. The timestamp audit in Appendix~\ref{app:solver-diag} confirms that Hydrostatic and MUSCL-HR frame $50$ are nearly time-aligned, while Boussinesq frame $50$ is at about half the shallow-water physical time.

Table~\ref{tab:solver-validation} summarizes the final solver health and consistency checks. The 24-sample lake-at-rest tests preserve roundoff, all $13{,}500$ stored dynamic rollouts per solver are finite, and Boussinesq has zero recorded conjugate-gradient failures. With sponge disabled for the conservation diagnostic, the shallow-water references keep the mass/free-surface integral to about $2.6 \times 10 ^ {-16}$. The CFL rows are only a timestep-sensitivity diagnostic at matched benchmark end time: CFL $0.60$ remains finite on the subset but shows large final-field differences, so these numbers are not a formal convergence proof. Taken together, these are sanity, sensitivity, and health checks, not substitutes for a convergence study, external solver comparison, observed-event validation, or production-code validation.

\begin{table}[pos=htbp]
\centering
\caption{Reference-solver sanity/health-check summary. Lake-at-rest and conservation checks are rerun diagnostics on denormalized inputs; dynamic finite-state statistics are aggregated over the generated datasets. CFL sensitivity is diagnostic only, not a formal convergence result.}
...
\begin{tabular}{p{0.34\linewidth}p{0.25\linewidth}cc}
    ...
    Lake at rest max $\eta$ drift           & MUSCL-HR    & $24$ & $5.33 \times 10 ^ {-15}$ \\
...
Table~\ref{tab:accuracy} reports direct single-pass accuracy on the hydrostatic reference, plus the FNO trained against each reference. Among the single-run direct hydrostatic configurations tested, FFNO is strongest, reaching relative $L_2=0.168$ with $0.411$M parameters, compared with $0.235$ and $5.443$M for the base FNO (about $13 \times$ fewer). The base FNO, U-FNO, and FNO-modes20 are effectively tied around $0.235$; FNO-modes8 is slightly worse at $0.241$; and WNO, U-Net, and CNN trail at $0.367$, $0.475$, and $0.607$. The ordering supports global operator structure, but the small gap between Fourier-family variants suggests the main gain is not simply from increasing retained modes.

\begin{table}[pos=htbp]
\centering
\caption{Same-solver field accuracy on the held-out test split ($N = 2500$). Parameter counts are in millions. MAE and RMSE are in denormalized benchmark-scale surface-elevation units; relative $L_2$ is dimensionless; max-err is the global worst-case element in normalized units. The first block shares the hydrostatic reference; the last two rows are the FNO trained and scored against the MUSCL-HR and Boussinesq references on their own test splits.}
\label{tab:accuracy}
...
Across references, FNO relative $L_2$ stays between $0.21$ and $0.26$. It is lowest on the Boussinesq target ($0.210$), but this ranking is unit-dependent: in denormalized benchmark-scale RMSE, the Boussinesq surrogate is the \emph{least} accurate same-solver FNO run ($0.00468$ vs $0.00227$ hydrostatic and $0.00281$ MUSCL-HR). The dimensionless relative $L_2$ and normalized-unit max-err metrics are affected by the Boussinesq target normalization scale, about $2.3 \times$ larger ($0.0221$ vs $0.0097$). We therefore avoid calling any single reference ``best'' outright: the FNO emulates all three to comparable dimensionless fidelity, while the denormalized benchmark-scale RMSE ranking favors the shallow-water targets. These are same-solver numbers, each surrogate scored against the reference it was trained on, so they measure emulation quality and are kept separate from the raw solver-vs-solver gaps in Section~\ref{subsec:res-solver-fidelity}.

\begin{figure}[pos=htbp]
\centering
\includegraphics[width=0.7\linewidth]{figures/hydrostatic_rollout.pdf}
\caption{Single-pass FNO rollout on one hydrostatic test scenario. The top row shows de-normalized bathymetry, source, and metadata; lower rows show reference $\eta$, prediction, and denormalized benchmark-scale absolute error for early, middle, and late frames. Reference and prediction share each frame's symmetric $\eta$ scale; the changing scale reflects amplitude decay over the horizon, and late-frame error growth matches Table~\ref{tab:perframe}.}
\label{fig:rollout}
\end{figure}

\paragraph{Per-frame error growth.}
The single-pass models predict all $50$ frames in one shot, but their accuracy is not uniform over the forecast horizon, as the qualitative rollout in Figure~\ref{fig:rollout} already suggests. Table~\ref{tab:perframe} reports the per-frame relative $L_2$ at the first, middle, and last frame for the FNO on each reference and for the stronger hydrostatic FFNO. On the hydrostatic target the FNO error rises over the horizon, from $0.214$ at the first frame to a peak of $0.355$ late in the rollout (around frame $38$), then eases slightly to $0.309$ at the final frame, so the last frame is somewhat below the mid-to-late peak rather than the maximum. The FFNO lowers the whole curve, from $0.135$ at the first frame to $0.273$ at the last, but its peak still rises to $0.289$, so factorization reduces rather than removes drift. On MUSCL-HR the FNO error grows more steeply and does not recover, from $0.217$ to $0.469$ at the last frame -- the worst late-horizon drift of the three. This is a limitation of a horizon-predicting surrogate: even without an explicit autoregressive feedback loop, late frames depend on longer-range propagation and accumulate more error, the same drift pattern a recurrent rollout would show. The Boussinesq FNO is the clear exception: its per-frame error is essentially flat, $0.207$ at the first frame and $0.208$ at the last (range $0.205$--$0.217$), showing no temporal drift over the horizon. We attribute this to the smoothness of the weakly dispersive elevation field discussed above rather than to a different model, and we return to the drift-targeting windowed forecaster in Section~\ref{subsec:res-window}.
...
\begin{table}[pos=htbp]
\centering
\caption{Per-sample runtime and speedup. Surrogate inference is full-trajectory GPU time (CUDA, fp32); solver rollout is CPU time (float64) over $8$ scenarios with $3$ repeats, excluding bathymetry/source generation. Speedup is solver time divided by inference time, an implementation-level comparison with unoptimized CPU references (Section~\ref{subsec:runtime}).}
\label{tab:speed}
\begin{tabular}{lccrr}
    \toprule
...
\begin{table}[pos=htbp]
\centering
\caption{Source-family breakdown for the hydrostatic FNO (per-sample-then-grouped, Section~\ref{subsec:generalization-protocol}). MAE is denormalized benchmark-scale; rel-$L_2$ is dimensionless; max-err is in normalized target units. rel-$L_2$ and max-err are per-sample means within the group; $n$ is the group size.}
\label{tab:ood}
\begin{tabular}{lcccc}
    \toprule
...
We additionally evaluate three named shift suites for the hydrostatic FNO: the \texttt{multi-gauss} subset, the \texttt{trench} subset, and the high source-strength tail ($s \ge 0.82$). These give per-sample-mean relative $L_2$ of $0.232$ and $0.267$ for the first two; within the high source-strength tail the per-family error spans $0.247$--$0.345$, with the \emph{rough} family again the worst ($0.345$) and the others between $0.247$ and $0.280$. As in the same-solver breakdown, rough sources are the hardest case and sit above the in-distribution band, while the remaining families stay within or just above it. We stress that these suites are carved from the in-distribution test pool by metadata filtering, not held out from training; following the distinction in Section~\ref{subsec:generalization-protocol} they are therefore \emph{difficult-subgroup} tests, not \emph{held-out-family} tests, and are evidence of robustness within the training distribution rather than of generalization to an unseen family.

Table~\ref{tab:strict-holdout} reports the stricter held-out-family stress tests. In each row the named family is removed before preprocessing, normalization statistics are refit from the filtered training split only, and the trained FNO is evaluated both on the in-distribution remainder and on the excluded family. For cross-model comparability, all values in this table are relative $L_2$ errors after reversing each checkpoint's own target standardization. The full-distribution checkpoint receives the same physical held-out inputs rebased from the filtered statistics to its original full-training input statistics before inference. Holding out \texttt{trench} is mild ($1.07 \times$ degradation over the filtered in-distribution test), \texttt{continental} and \texttt{okada-like} are moderate ($1.48 \times$ and $1.31 \times$), and \texttt{rough} sources are the clear failure mode ($2.54 \times$). The full-distribution model is more accurate than the strict model on all four held-out subsets, consistent with a benefit from having seen each family during training; this is a single-run reference comparison rather than a causal estimate of the value of one family.

\begin{table}[pos=htbp]
\centering
\caption{Strict held-out-family hydrostatic FNO stress tests. ID is the filtered in-distribution test remainder for the model trained without the named family; held-out is the excluded family; full model is the main FNO evaluated on the same physical held-out subset after rebasing inputs to the main model's training normalization. Relative $L_2$ values are dimensionless and computed after target denormalization so the two checkpoints are compared in one benchmark-scale space. Ratio is held-out/ID for the strict model.}
\label{tab:strict-holdout}
\begin{tabular}{lcccc}
    \toprule
    Held-out family & ID rel-$L_2$ & Held-out rel-$L_2$ & Full model rel-$L_2$ & Ratio \\
    \midrule
    \texttt{trench} bathymetry      & $0.253$ & $0.272$ & $0.240$ & $1.07$ \\
    \texttt{continental} bathymetry & $0.233$ & $0.344$ & $0.257$ & $1.48$ \\
    \texttt{rough} source           & $0.233$ & $0.592$ & $0.365$ & $2.54$ \\
    \texttt{okada-like} source      & $0.234$ & $0.306$ & $0.220$ & $1.31$ \\
    \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[pos=htbp]
\centering
\caption{Proxy resolution-transfer for the hydrostatic FNO ($N=2500$). The same weights are evaluated on test fields resized to each grid. MAE/RMSE are denormalized benchmark-scale metrics; rel-$L_2$ is dimensionless; max-err is in normalized target units. The $64 \times 64$ row is the native training resolution and matches Table~\ref{tab:accuracy}.}
\label{tab:crossres}
\begin{tabular}{lcccc}
    \toprule
...
The operator runs at $32 \times 32$ without error and produces a coherent field, confirming the resolution-flexibility property. Accuracy degrades on the coarser grid (relative $L_2$ rising from $0.235$ to $0.420$), which is expected: downsampling to $32 \times 32$ removes the high-wavenumber content that the field carries at $64 \times 64$. This proxy is therefore only a cheap resizing diagnostic, not a like-for-like simulation-resolution result. No proxy $128 \times 128$ row is reported because the current result archive contains only the $32 \times 32$ resized proxy diagnostic; the $128 \times 128$ setting is covered by the native-resolution diagnostics below. This is exactly why Section~\ref{subsec:generalization-protocol} separates the proxy from the \emph{native} setting, where the wavefield is physically resolved at each resolution.
...
\begin{table}[pos=htbp]
\centering
\caption{Zero-shot native-resolution transfer for hydrostatic FNO checkpoints. Rows give the training grid and columns the native evaluation grid; all checkpoints use the matched $700$-train native protocol, distinct from the main $10{,}000$-train $64 \times 64$ FNO. Entries are relative $L_2$ on the $150$-scenario test split.}
...
The main real-bathymetry diagnostic uses a 10-crop GEBCO-derived morphology suite covering fully wet offshore regions including Sumatra--Java, Alaska--Aleutian, Cascadia, the Hellenic Arc/Aegean, Florida--Bahamas, the Caribbean, the Philippines east trench, Kuril--Japan, Tohoku offshore, and Nankai. The real-bathymetry diagnostic crops are derived from the GEBCO\_2026 Grid, a global ocean-and-land terrain model provided at 15 arc-second resolution \citep{gebco2026}; after cropping, they are resampled to the model's $64 \times 64$ grid and linearly rescaled to the fully wet training range $[-10,\, -0.75]$. The raw files contain bathymetry only, so each crop is paired with a synthetic initial-displacement source, advanced with the hydrostatic solver, and preprocessed with the main hydrostatic training normalization. This is therefore a synthetic-source/real-bathymetry morphology-transfer suite, not real-event validation.

\begin{table}[pos=htbp]
\centering
\caption{Synthetic-source transfer on the 10-crop fully wet real-bathymetry morphology suite. Direct models use static inputs; windowed models are first-target-frame-seeded diagnostics scored on the remaining $49$ predicted target frames, so they are not like-for-like with direct $50$-frame prediction. Relative $L_2$ is shown in normalized and denormalized benchmark-scale forms.}
\label{tab:real-bath}
\begin{tabular}{lccc}
    \toprule
    Model & Samples & rel-$L_2$ & denorm. rel-$L_2$ \\
    \midrule
...
FFNO improves the direct FNO on this suite from rel-$L_2 = 0.275$ to $0.183$. The windowed scores are lower (Window-FNO-5 $0.053$, Window-FFNO-5 $0.039$), but they use the first-target-frame-seeded diagnostic of Section~\ref{subsec:res-window}, are scored on the remaining $49$ predicted target frames, and are not like-for-like with direct $50$-frame prediction. We therefore claim only transfer to fully wet real bathymetric morphologies under synthetic forcing, not real-event or site-specific hazard skill. Figure~\ref{fig:real-bath} shows one illustrative Tohoku offshore crop; the coastline variants are in Appendix~\ref{app:details}.

\begin{figure}[pos=htbp]
\centering
\includegraphics[width=0.7\linewidth]{figures/real_bathymetry_offshore.pdf}
\caption{Synthetic-forcing morphology transfer on one GEBCO-derived Tohoku crop rescaled to $[-10,\, -0.75]$. Top: bathymetry, synthetic source, and hydrostatic reference $\eta$ at frame 49. Bottom: direct-FNO error, Window-FNO-5 error, and Window-FNO-5 $\eta$ on the reference scale. Panel rel-$L_2$ labels are single-crop trajectory scores for this illustrative case, not the 10-crop aggregate in Table~\ref{tab:real-bath}; this is not real-event validation.}
\label{fig:real-bath}
\end{figure}

\subsection{Solver-gap and cross-solver diagnostics}
\label{subsec:res-solver-fidelity}

Because the training solver is not itself ground truth, Section~\ref{subsec:cross-solver} separates two quantities: the raw denormalized benchmark-scale gap between two numerical references on shared scenarios, and the surrogate error of a model trained on one reference and scored against another. Table~\ref{tab:solvergap} reports the raw solver-vs-solver gaps on all $2{,}500$ shared test scenarios, in both directions for each pair. These are same-scenario, same-saved-index diagnostics on the released labels, not strict common-physical-time solver-fidelity comparisons. The two shallow-water references are close: hydrostatic versus MUSCL-HR has a mean relative $L_2$ of $0.164$ (RMSE $0.00173$). Both shallow-water references are far from the weakly dispersive Boussinesq comparison reference: hydrostatic versus Boussinesq has a mean relative $L_2$ of $0.861$ and MUSCL-HR versus Boussinesq $0.897$, an order of magnitude larger in RMSE ($\approx 0.016$--$0.017$). The reverse Boussinesq-vs-shallow-water directions give larger relative $L_2$ ($1.60$ and $1.55$). We interpret this only as directional normalization: each row divides the same symmetric difference by the norm of the row's reference field, so the two directions answer different ``relative to which reference?'' questions. The symmetric RMSE and MAE are therefore the safer scale comparisons. This gap structure is consistent with the different numerical references, but because Boussinesq has a different saved-frame physical-time scale, we do not read the cross-family rows as strict common-physical-time fidelity errors.

\begin{table}[pos=htbp]
\centering
\caption{Raw solver-vs-solver gaps on shared test scenarios ($N = 2500$, field $\eta$ trajectory), compared as same-scenario, same-saved-index released labels. RMSE and MAE are denormalized benchmark-scale metrics; relative $L_2$ is a per-sample mean. The relative $L_2$ is asymmetric because it normalizes by the second solver's field norm; RMSE and MAE are symmetric.}
\label{tab:solvergap}
\begin{tabular}{llccc}
    \toprule
...
The cross-reference diagnostic checks both the target denormalization signature and the input-channel normalization statistics between the prediction dataset and the reference dataset. For all four pairs the input-channel signature check passes: the bathymetry and source channels share identical normalization statistics across the prediction and reference datasets (the \texttt{initial\_depth} channel is in stored scale on both sides), so the surrogate sees inputs on the same scale it was trained on. The target denormalization is applied separately to prediction and reference using each dataset's own stored statistics, so the denormalized benchmark-scale numerator and denominator are each well defined in their native scale. The ratios are therefore reported as direct denormalized benchmark-scale comparisons, and the qualitative conclusion ($\rho>1$ throughout) is unaffected.
...
Table~\ref{tab:window} compares the windowed models against their direct single-pass counterparts on the hydrostatic test split. The windowed rows are first-target-frame-seeded diagnostics scored on the remaining $49$ predicted target frames, not like-for-like direct $50$-frame predictions. Window-FNO-5 reaches a full-trajectory relative $L_2$ of $0.077$, against $0.235$ for the single-pass FNO. Window-FFNO-5 is lower again at $0.052$ (denormalized benchmark-scale rel-$L_2 = 0.0513$), compared with $0.168$ for the direct FFNO. The same drift pattern remains: the windowed FFNO starts at $0.030$ and rises to $0.169$ by the final predicted frame, so it reduces but does not eliminate temporal accumulation.

\begin{table}[pos=htbp]
\centering
\caption{Windowed versus direct hydrostatic models on the test split ($N = 2500$). Params are in millions; full-traj is global relative $L_2$; per-frame columns are relative $L_2$ at the first scored prediction, mid horizon, and final frame. Direct models predict all $50$ target frames from static inputs; windowed models are first-target-frame-seeded diagnostics scored on the remaining $49$ predicted target frames, so the tasks are not like-for-like. Inference time is per sample on GPU.}
\label{tab:window}
\small
\begin{tabular}{lcccccc}
    \toprule
    Model & Params (M) & full-traj & first scored & mid & final & infer (ms) \\
    \midrule
...
\begin{tabular}{lccccc}
    ...
    high source-strength tail   & $0.376$ & $0.589$ & $0.673$ & $0.728$ & $0.700$ \\
    \bottomrule
\end{tabular}
\end{table}

Two findings stand out. First, the raw ensemble is \emph{overconfident}: empirical coverage remains below the nominal level at every interval, with the nominal $95\%$ interval capturing $83\%$ of in-distribution targets and $73$--$85\%$ under shift. Thus the uncalibrated Gaussian intervals built from ensemble variance are too narrow and should not be read as calibrated probabilities. Second, and more positively, the error--uncertainty correlation is stable and moderately strong across all settings ($0.65$--$0.70$), and is highest on the high source-strength tail ($0.700$). So even though the raw scale of the uncertainty is miscalibrated, its \emph{spatial structure} tracks where the model is actually wrong, which is the property that matters for flagging untrustworthy regions of a prediction.

The behavior under shift is mixed. Raw coverage does not improve uniformly under distribution shift, and on the high source-strength tail the normalized Gaussian negative log-likelihood is worse than in distribution ($-0.22$ versus $-1.91$), meaning the variance still fails to grow enough to match the larger errors in that tail. The \texttt{trench} bathymetry suite is the one raw case where coverage edges above the in-distribution level, but not enough to reach the nominal targets. Appendix~\ref{app:add-result-tab} reports the full reliability/calibration diagnostic: a validation-only scalar variance inflation ($\gamma = 3.359364$) improves marginal coverage on the in-distribution test split and some OOD suites, while the hardest high source-strength tail remains under-covered. We therefore treat these as reliability diagnostics rather than calibrated risk estimates: the ensemble produces a useful relative ranking of where error concentrates, and the post-hoc correction improves interval scale, but per-element marginal coverage over correlated pixels and frames is not operational risk calibration.
