...
The FNO behaves consistently across the three references, with relative $L_2$ between $0.21$ and $0.26$. On the dimensionless relative $L_2$ it scores lowest on the Boussinesq target ($0.210$). This ranking is unit-dependent and should be read with care: in denormalized benchmark-scale surface-elevation units the Boussinesq surrogate is actually the \emph{least} accurate of the three FNO same-solver runs (RMSE $0.00468$ vs $0.00227$ hydrostatic and $0.00281$ MUSCL-HR), and likewise has the largest denormalized maximum error. The dimensionless and normalized-unit metrics flatter Boussinesq because its target normalization scale is about $2.3 \times$ larger ($0.0221$ vs $0.0097$), which shrinks every error expressed relative to that scale. We therefore avoid calling any single reference ``best'' outright: the FNO emulates all three to a comparable dimensionless fidelity, and the denormalized benchmark-scale ranking favors the shallow-water targets. These are same-solver numbers, each surrogate scored against the reference it was trained on, so they measure emulation quality and are kept separate from the raw solver-vs-solver gaps in Section~\ref{subsec:res-solver-fidelity}.
...
\begin{table}[!htbp]
\centering
\caption{Source-family breakdown for the hydrostatic FNO (per-sample-then-grouped, Section~\ref{subsec:generalization-protocol}). MAE is denormalized benchmark-scale; rel-$L_2$ and max-err are per-sample means within the group; $n$ is the group size.}
\label{tab:ood}
\begin{tabular}{lcccc}
    \toprule
...
\begin{table}[!htbp]
\centering
\caption{Proxy resolution-transfer for the hydrostatic FNO ($N=2500$). The same weights are evaluated on test fields resized to each grid. MAE/RMSE are denormalized benchmark-scale metrics; rel-$L_2$ is dimensionless. The $64 \times 64$ row is the native training resolution and matches Table~\ref{tab:accuracy}.}
\label{tab:crossres}
\begin{tabular}{lcccc}
    \toprule
...
\begin{table}[H]
\centering
\caption{Synthetic-source transfer on the 10-crop fully wet real-bathymetry morphology suite. Direct models use static inputs; windowed models are seeded with the first target frame and score the remaining $49$ predicted target frames, so they are not like-for-like. Relative $L_2$ is shown in normalized and denormalized benchmark-scale forms.}
\label{tab:real-bath}
\begin{tabular}{lccc}
    \toprule
...
FFNO improves the direct FNO on this suite from rel-$L_2 = 0.275$ to $0.183$. The windowed scores are lower (Window-FNO-5 $0.053$, Window-FFNO-5 $0.039$), but they use the seeded rollout of Section~\ref{subsec:res-window} and are not like-for-like. We therefore claim only transfer to fully wet real bathymetric morphologies under synthetic forcing, not real-event or site-specific hazard skill. Figure~\ref{fig:real-bath} shows one illustrative Tohoku offshore crop; the coastline variants are in Appendix~\ref{app:details}.
...
\begin{table}[H]
\centering
\caption{Windowed versus direct hydrostatic models on the test split ($N = 2500$). Params are in millions; full-traj is global relative $L_2$; per-frame columns are relative $L_2$ at the first scored prediction, mid horizon, and final frame. Direct models predict all $50$ target frames from static inputs; windowed models are seeded with the first target frame and score the remaining $49$ predicted target frames, so the tasks are not like-for-like. Inference time is per sample on GPU.}
\label{tab:window}
\small
