...
DEFAULT_CONFIG = Path("configs/eval/common_time_v2_h1.yaml")
DEFAULT_H0 = Path(
...
DEFAULT_OUTPUT_BASE = Path("artifacts/common_time_v2/h1")
...
def _configure_threads(workers: int) -> None:
    ...
    if invalid:
        ...
        raise SystemExit(
            "H1 multiprocessing requires single-thread numerical backends; "
            f"found {details}"
        )
    for key in THREAD_ENV_KEYS:
...
def _show_progress(event: dict[str, object]) -> None:
    if event["event"] == "start":
        ...
        print(
            f"[H1] {event['completed']}/{event['total']} "
        ...
        print(
            f"[H1] finalized decision={event['decision']} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Freeze, execute, resume, and verify the common-time-v2 H1 "
            "implementation/health smoke."
        )
    )
    ...
    if args.action == "status":
        print(json.dumps(h1_status(args.contract_root), indent=2, sort_keys=True))
        return
    if args.action == "checksums":
        validate_h1_checksums(args.contract_root)
        print(f"checksums valid: {args.contract_root}")
        return

    _configure_threads(args.workers)
    result = execute_h1_contract(
        repo_root=ROOT,
